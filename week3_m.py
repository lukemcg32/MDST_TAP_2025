...
train_dataset = MNISTCSVLoader("US_Accidents_Cleaned.csv")
test_dataset = MNISTCSVLoader("US_Accidents_Cleaned.csv")
...
class NeuralNet(nn.Module):
    def __init__(self):
        ...
        # Create the first fully connected layer (input: 28*28 pixels, output: 128 neurons)
        self.fc1 = nn.Linear(784, 128)
        # Create the second fully connected layer (input: 128 neurons, output: 64 neurons)
        self.fc2 = nn.Linear(128, 64)
        # Create the third (output) fully connected layer (input: 64 neurons, output: 10 classes)
        self.fc3 = nn.Linear(64, 10)
        # Define the ReLU activation function
        self.relu = nn.ReLU()

    def forward(self, x):
        # If x is not flattened (e.g., shape: [batch_size, 28, 28]), flatten it
        if len(x.shape) > 2:
            x = x.view(x.size(0), -1)
        # Apply the first fully connected layer and pass it through ReLU activation
        x = self.relu(self.fc1(x))
        # Apply the second fully connected layer and pass it through ReLU activation
        x = self.relu(self.fc2(x))
        # Apply the final output layer (do not use activation here; CrossEntropyLoss expects raw logits)
        x = self.fc3(x)
        return x
...
with torch.no_grad():
    for images, labels in test_loader:
        ...
        # Collect correctly classified images (up to 10 examples)
